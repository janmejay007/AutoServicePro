...
const CustomerTables = ({ user }) => {
  ...
  const fetchBookings = async (email) => {
    try {
      const response = await axios.get(`http://localhost:8181/cutomers/${email}`);
  ...
  const cancelBooking = async (bookingToCancel) => {
    try {
      ...
      const resp = await axios.put(
        `http://localhost:8181/bookings/${updatedBooking.bookingId}`,
        updatedBooking
      );
  ...
  const fetchPayments = async (bookingIds) => {
    try {
      const promises = bookingIds.map(async (id) => {
        const response = await axios.get(`http://localhost:8181/payments/${id}`);
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:8181/payments/payment?bookingId=${selectedPayment.paymentId}&userId=${user.userId}&paymentMode=${paymentMode}`);
  ...
  const submitFeedback = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:8181/feedbacks', newFeedback);
